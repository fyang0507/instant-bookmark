...
const BookmarkForm: React.FC = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setFile(null); // Explicitly clear if no file is chosen
      // Reset input value to allow re-selection of the same file after cancelling
      if (e.target) {
          e.target.value = '';
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAutoGenerated) {
      ...
      if (url && !file) {
        setPreview({ title: manualTitle, summary: manualSummary });
        return;
      }
    }
    ...
    try {
      ...
      if (file) {
        result = await processScreenshot(file, isAutoGenerated, manualTitle, manualSummary);
        ...
        if (isAutoGenerated) {
            ...
            console.warn('[BookmarkForm] Reached unexpected state in handleSubmit for URL processing with auto-generation off and no file.');
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="flex flex-col space-y-6">
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex flex-col justify-center items-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-700 border-dashed rounded-md flex-grow">
            ...
            <div className="flex justify-end pt-4">
              <Button
                type="submit"
                disabled={(!url && !file && isAutoGenerated) || (!manualTitle.trim() && !manualSummary.trim() && !isAutoGenerated) || isProcessing}
                isLoading={isProcessing && !preview}
              >
                {isAutoGenerated || file ? 'Process' : 'Preview'}
              </Button>
            </div>
          </div>
        </form>
